...
from nibelungenbruecke.scripts.database.database_management import Database
...
class DigitalTwin:
    ...
    def predict(self, model_to_run, api_key, orchestrator_simulation_parameters, UQ_flag, dimension_change_flag=None):
        """
        ...
        if updated:
            ...
            self._run_model(api_key, orchestrator_simulation_parameters, env_params)
            # Updating model parameters with values from the last time step, excluing "time"
            row = env_params.iloc[-1].drop("time")
            self._loaded_params["generation_models_list"][0]["model_parameters"]["thermal_model_parameters"].update(
                row.to_dict()
            )
            self._update_cached_model(self._loaded_params, updated_params)
    ...
    def _get_or_load_parameters(self, dimension_change_flag=None):
        """
        ...
        if not self.cache_object.cache_model:
            ...
            if self.cache_object.model_name != self.cache_model_name or dimension_change_flag:
                parameters = self.cache_object.load_cache(self.cache_model_path, self.cache_model_name)
                if not parameters:
                    with open(self.model_parameters_path, 'r') as file:
                        parameters = json.load(file)
    ...
    def _initialize_default_model(self, api_key, orchestrator_simulation_parameters):
        ...
        for model in self._models:
            ...
            if "TransientThermal" in self.model_to_run:
                model_path_option = orchestrator_simulation_parameters.get('model_info')['path']
                model_dim_option = orchestrator_simulation_parameters.get('model_info')['type']
                if '2D' in model_dim_option:
                    if "Pilot" in model_path_option:
                        model_path = model_paths["transientthermal_model_path_pilot"]
                    elif "Span" in model_path_option:
                        model_path = model_paths["transientthermal_model_path_span"]
                elif '3D' in model_dim_option:
    ...
    def _run_model(self, api_key, orchestrator_simulation_parameters, env_params):
        """
        ...
        self.initial_model.solve(api_key, orchestrator_simulation_parameters, env_params)
    ...
    def virtual_sensor_load(self, orchestrator_simulation_parameters):
        ...
        if "Thermal" in model:
            model_path = orchestrator_simulation_parameters.get('model_info')['path']
            model_dim = orchestrator_simulation_parameters.get('model_info')['type']
            if '2D' in model_dim:
                if "Pilot" in model_path:
                    path = self.orchestrator_parameters["model_path"][0]["transientthermal_model_path_pilot"]
                elif "Span" in model_path:
                    path = self.orchestrator_parameters["model_path"][0]["transientthermal_model_path_span"]
                    
            elif '3D' in model_dim:
        ...
        if np.all(geometry[:, 2] == 0):     ##NOTE: for 2D geometries. ~0.1
            
            min_list = []
            for i in geometry:
                distances = np.linalg.norm(geometry - i, axis=1)
                distances = distances[distances > 0]   # remove self-distance
                min_dist = np.min(distances)
                min_list.append(min_dist)
            
            threshold = np.max(min_list)
